  ...
  <div class="env-setup-panel">
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/:id/prepare</p>
          <p class="description">
            Combines context to auto-extract entities and relations from the knowledge graph, initializes simulation agents with unique behaviors and memories
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/:id/prepare</p>
          <p class="description">
            LLM intelligently sets world time flow, recommendation algorithms, activity windows, posting frequency, and event triggers
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/:id/prepare</p>
          <p class="description">
            Based on narrative direction, auto-generate initial activation events and hot topics to guide the simulation's initial state
          </p>
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                ...
                <span class="section-desc">SmartFish auto-planned <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours, each round = <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> min real time</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>Est. ~{{ Math.round(customMaxRounds * 0.6) }} min (100 agents)</span>
                  </div>
                </div>
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">For first run, switch to 'Custom' to reduce rounds for a quick preview</p>
                    </div>
                  </div>
                </div>
              </div>
...
const startPrepareSimulation = async () => {
  ...
  try {
    // SmartFish API: prepareSimulation(simulationId, data)
    const res = await prepareSimulation(props.simulationId, {
    ...
    if (res.success && res.data) {
      ...
      if (res.data.expected_entities_count) {
        ...
        addLog(`Found ${res.data.expected_entities_count} entities from graph`)
...
const pollPrepareStatus = async () => {
  ...
  try {
    // SmartFish API: getPrepareStatus(simulationId, taskId)
    const res = await getPrepareStatus(props.simulationId, taskId.value)
    ...
    if (res.success && res.data) {
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.summary) {
          ...
          addLog(`  Platforms: Twitter ${data.summary.has_twitter_config ? 'Y' : 'N'}, Reddit ${data.summary.has_reddit_config ? 'Y' : 'N'}`)
        }
